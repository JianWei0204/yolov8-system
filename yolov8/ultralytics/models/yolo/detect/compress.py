...
def get_pruner(opt, model, example_inputs):
    ...
    for k, m in model.named_modules():
        if isinstance(m, Detect):
    #         #     ignored_layers.append(Detect)
    #         #     ignored_layers.append(m.cv2)
    #         ignored_layers.append(m.stem)
    #         ignored_layers.append(m.cv2)
    #         ignored_layers.append(m.cv3)
    #         ignored_layers.append(m.dfl)


            # ignored_layers.append(m.cv2[0])
            # ignored_layers.append(m.cv2[1])
            # ignored_layers.append(m.cv2[2])
            # # ignored_layers.append(m.cv2[3])
            # ignored_layers.append(m.cv3[0])
            # ignored_layers.append(m.cv3[1])
            # ignored_layers.append(m.cv3[2])
            # # ignored_layers.append(m.cv3[3])
            # ignored_layers.append(m.dfl)
            # ignored_layers.append(m.stem)
            # ignored_layers.append(m.stride)

            # ignored_layers.append(m)
            ignored_layers.append(m.cv2)
            ignored_layers.append(m.cv3)
            ignored_layers.append(m.dfl)
        if isinstance(m, Partial_conv3):
            ignored_layers.append(m)
